...
def _open_reader(bag_dir: str):
    ...
    for storage_id in ("sqlite3", "mcap"):
        ...
        try:
            ...
            last_err = e
    raise RuntimeError(f"Could not open bag '{bag_dir}': {last_err}")
...
class SceneBagRecorder(Node):
    ...
    def _convert_and_cleanup(self, bag_dir: Path, qos_override_path: str | None):
        try:
            ...
            if topic_types[self.topic] != "sensor_msgs/msg/Image":
                raise RuntimeError(
                    f"Topic {self.topic} has type {topic_types[self.topic]}, expected sensor_msgs/msg/Image"
                )
